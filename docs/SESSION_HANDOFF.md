...
- **Inventarisasi Tier Agen**: entitas `Agen` bernama (2–3 per Gudang Wilayah), baris stok per (Agen × Produk), migrasi data `Tier.Agen`→`GudangWilayah`, mock agen (50% stok gudang dibagi rata + DOT rata, audit Transfer), identitas agen Create/Update = Superadmin+Supervisi, halaman Daftar Agen + Detail Agen + Update Data Harian.
- **Transfer Gudang Wilayah → Agen**: modal "Kirim ke Agen" (Superadmin+Supervisi) di DetailSalesArea, pilih 1 agen + kuantitas per SKU (3 SKU LPG sekaligus / 1 input minyak), loop `Transfer` atomic per SKU via `AgenService.TransferFromWarehouseAsync`. **Fix bug**: guard overdraft di jalur `Transfer` salah (`EnsureSufficientStock(entity, kuantitas)` memeriksa `kuantitas<0` → selalu lolos); diperbaiki jadi `entity.Stok - kuantitas`.
...
- Domain: `Entities/{Agen,Wilayah,Produk,Tier,StokEntitas,RencanaKedatangan,StockTransaction,StockTransactionRecord,AuditLog}.cs`, `Services/StockCalculator.cs`, `Abstractions/IAuditLogService.cs`.
- Infrastructure: `Persistence/ApplicationDbContext.cs`, `Persistence/Migrations/*` (5 migrasi — `AddAgenAndGudangWilayahTier`), `Seed/{SeedData,ExcelStockSeeder,AgenMockSeeder}.cs` (seed LPG dari Excel + **mock minyak tanah 7 wilayah + mock agen 18**), `Excel/XlsxReader.cs`, `Services/{StockWriteService,StockDashboardService,AgenService,AuditLogService,UserAdminService,ActiveRoleClaimsPrincipalFactory}.cs`.
...
- **Model domain**: entitas per `(Wilayah × Produk × Tier)`; CD = Stok÷DOT; Status Kritis<3/Warning<7/Aman≥7; `CD_n = (sisa@ETA + NextSupply)÷DOT` (**konseptual, JANGAN tiru rumus Excel**); konservasi stok via transaksi atomic (Receive/Issue/Adjust±/Transfer), `Issue` auto `StokHabisTerjual`, tolak overdraft, audit log tiap mutasi.
